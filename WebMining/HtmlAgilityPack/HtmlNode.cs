    ...
    public class HtmlNode : IXPathNavigable
    {
        ...
        public void WriteTo(TextWriter outText, bool isCData)
        {
            ...
            switch (_nodetype)
            {
                ...
                case HtmlNodeType.Text:
                    ...
                    if (isCData)
                    {
                        outText.Write(HtmlDocument.RemoveIllegalCDataChars(html));
                    }
                    else if (_ownerdocument.OptionOutputAsXml)
                    {
                        outText.Write(HtmlDocument.EncodeXmlText(html));
                    }
                    else if (_ownerdocument.OptionNormalizeHtmlEncoding)
                    {
        ...
        internal void WriteAttribute(TextWriter outText, HtmlAttribute att)
        {
            ...
            if (_ownerdocument.OptionOutputAsXml)
            {
                ...
                outText.Write(" " + name + "=" + quote + HtmlDocument.EncodeXmlAttrVal(att.XmlValue) + quote);
            }
            else
            {
                if (_ownerdocument.OptionOutputUpperCase)
                ...
                string attValue = _ownerdocument.OptionNormalizeHtmlEncoding ? HtmlDocument.EncodeHtmlAttrVal(att.Value) : att.Value;
                if (_ownerdocument.OptionOutputOptimizeAttributeValues)
                {
                    if (att.Value.IndexOfAny(new Char[] {(char) 10, (char) 13, (char) 9, ' '}) < 0)
                    {
                        outText.Write(" " + name + "=" + attValue);
                    }
                    else
                    {
                        outText.Write(" " + name + "=" + quote + attValue + quote);
                    }
                }
                else
                {
